...
| **devise** | ✅ verified (partial) | mattr_accessor (mostly) + Warden closures | yes (with Warden patch) | Config is mattr → shim fixes. **Warden::Hooks lazy class ivars** (`@_on_request ||= []` etc.) patched in Phase 4: workers treat callback arrays as empty (correct for read-only shared app). Full-stack app with Devise dispatches `GET /up` → 200. |

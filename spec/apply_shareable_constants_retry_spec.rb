...
#
# _apply_shareable_constants! sets @applied = true after the
...
class ApplyShareableConstantsRetrySpec < Minitest::Spec
  ...
  def setup
    ...
    @saved_flag = RactorRailsShim::ConstantShareabilizer.applied?
  ...
  def teardown
    ...
    if @saved_flag
      RactorRailsShim::ConstantShareabilizer.instance_variable_set(:@applied, true)
    else
      RactorRailsShim::ConstantShareabilizer.reset_applied!
    end
    super
  ...
  it "does NOT set the done flag when a registered constant is undefined" do
    ...
    # should leave @applied unset to allow a later retry.
